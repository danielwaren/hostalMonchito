...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: any }) {
  if (active && payload && payload.length > 0) {
    const { descripcion, cantidad, totalAbonos } = payload[0].payload
    return (
      <div className="rounded-md bg-white dark:bg-zinc-900 p-3 text-sm text-black dark:text-white shadow-lg border dark:border-zinc-700">
        ...
        <div>Total abonos: ${totalAbonos.toLocaleString("es-CL")}</div>
      </div>
    )
  }
  return null
...
export function GraficoDescripciones() {
    ...
    const obtenerDatosCartola = async () => {
      try {
        ...
        const descripcionStats = data?.reduce(
          (
            acc: Record<string, { cantidad: number; totalAbonos: number }>,
            item: CartolaData
          ) => {
            const descripcion = item.Descripcion.trim()
            const abono = Number(item.Abonos) || 0
            if (!acc[descripcion]) {
              acc[descripcion] = { cantidad: 1, totalAbonos: abono }
            } else {
              acc[descripcion].cantidad += 1
              acc[descripcion].totalAbonos += abono
            }
            return acc
          },
          {}
        ...
        const chartDataFormatted = Object.entries(descripcionStats)
          .filter(([_, { totalAbonos }]) => totalAbonos > 0)
          .map(([descripcion, { cantidad, totalAbonos }]) => ({
            descripcion,
            cantidad,
            totalAbonos,
            fill: getRandomColor(),
  ...
  // Función para cargar detalle con paginación
  const cargarDetallePagina = async (descripcion: string, pagina: number) => {
    ...
    try {
      // Obtener total registros para la descripcion
      ...
      const { data, error } = await supabase
        .from("Cartola")
        .select("id, Fecha, Descripcion, Abonos, Operacion")
        .eq("Descripcion", descripcion)
    ...
    return (
      <Card className="flex flex-col w-full">
        ...
        <CardContent className="flex-1 pb-0">
          <div className="w-full h-[300px] flex items-center justify-center">
            <span className="text-lg font-medium text-muted-foreground">
              No se encontraron descripciones con abonos en la tabla Cartola
            </span>
          </div>
        </CardContent>
      </Card>
    ...
    <>
      <Card className="flex flex-col w-full">
        <CardHeader className="items-center pb-0">
          ...
          <CardDescription>
            Todas las descripciones de abonos registrados
          </CardDescription>
        </CardHeader>
        <CardContent className="flex-1 pb-0">
          <div className="w-full h-[300px] flex items-center justify-center">
        ...
        <CardFooter className="flex-col gap-2 text-sm">
          <div className="flex items-center gap-2 leading-none font-medium">
            Total de registros: {totalRegistros} <TrendingUp className="h-4 w-4" />
          </div>
          <div className="text-muted-foreground leading-none">
            Mostrando todas las descripciones con abonos registrados
          </div>
        </CardFooter>
      </Card>
                ...
                <table className="w-full border-collapse table-auto">
                  <thead>
                    <tr>
                      ...
                      <th className="border px-4 py-2 text-right">Abonos</th>
                    </tr>
                  </thead>
                  <tbody>
                    {detalleData.map(({ id, Fecha, Descripcion, Abonos, Operacion }) => (
                      <tr
                        key={id}
                      ...
                      >
                        ...
                        <td className="border px-4 py-2 text-right">
                          ${Number(Abonos).toLocaleString("es-CL")}
                        </td>
                      </tr>
                    ))}
                  </tbody>
